...
class CrashpadCard extends Component {
  ...
  showAvailability() {
    const locationFilterOptions = [
      ...
      {
        key: "dancingsunflower",
        text: "Dancing Sunflower",
        value: "ds"
      },
      {
        key: "wildflower",
    ...
    if (this.state.filters) {
      if (this.state.filters.crashpad) {
        email_code =
          "Preferred location: " +
          abbrevMap[this.state.filters.crashpad] +
          ". ";
      }
    ...
    return (
      <Modal
        ...
        trigger={<Button basic>Show availability!</Button>}
      >
        <Modal.Header>
          <Dropdown
            inline
            onChange={this.handleFilterChange.bind(this, "crashpad")}
            options={locationFilterOptions}
            defaultValue={locationFilterOptions[0].value}
          />
        </Modal.Header>
        <Modal.Content>
          <Modal.Description>
            <Header>
              <Dropdown
                ...
                upward
                inline
                onChange={this.handleFilterChange.bind(this, "room")}
                options={roomFilterOptions}
                defaultValue={roomFilterOptions[0].value}
              />
              <Dropdown
                className="margin"
                upward
                inline
                onChange={this.handleFilterChange.bind(this, "bunk")}
                options={bunkFilterOptions}
                defaultValue={bunkFilterOptions[0].value}
              />
            </Header>

            <FilterCalendar
              filters={this.state.filters}
              showReqBtn={this.showReqBtn.bind(this)}
            />
            {this.state.reqBtn ? (
              <Button
                onClick={() =>
                  (document.location.href = `mailto:${"G"}${
                    ("" + (1 === 2))[1]
                  }rr${("" + (1 === 2))[4]}tt.holm${
                    ("" + (2 === 2))[3]
                  }s+fn@gmail.com?subject=Future Flower Net Resident&body=${email_code +
                    this.state.reqBtn}`)
                }
                primary
              >
              ...
              <div>
                Choose your preferences, select a date, request your spot!
              </div>
            )}
          </Modal.Description>
        </Modal.Content>
      ...
      <div className="card">
        ...
        >
          ...
          <Card.Content extra href={this.props.photosURL}>
            <a href={this.props.photosURL}>
              <Icon name="photo" />See Photos!
            </a>
            <br />
            {this.showHotbed()}
          </Card.Content>
